#
#RUNWITH
# $fx=1032.4; $fy=1030.9; $cx=640.2; $cy=360.7 $imgs=Get-ChildItem .\calib -File | Where-Object { $_.Extension -match '^\.(jpg|jpeg|png)$' } | Select-Object -ExpandProperty FullName
# python3 .\apriltag.py -k "$fx,$fy,$cx,$cy" -s 0.05 $imgs
#
#
...
library used by ctypes.
    ...
    def __init__(self, options=None, searchpath=[]):
        ...
        # Initialize attributes early so __del__ is always safe, even on init errors.
        self.libc = None
        self.tag_detector = None
        self.options = options

        # Choose likely shared library names for the running OS.
        system_name = platform.system()
        if system_name == 'Windows':
            candidate_filenames = ['libapriltag.dll', 'apriltag.dll']
        elif system_name == 'Darwin':
            candidate_filenames = ['libapriltag.dylib']
        else:
        ...
        if self.libc is None:
            raise RuntimeError('could not find apriltag shared library; tried: ' + ', '.join(candidate_filenames))
    ...
    def __del__(self):
        if getattr(self, 'tag_detector', None) is not None and getattr(self, 'libc', None) is not None:
...
def _get_demo_searchpath():
    ...
    return [
        ...
        "~/projects/yaskawa/apriltag/build/lib"
    ]
...
def _draw_pupil_detections(overlay, detections):

    for detection in detections:
        corners = numpy.asarray(detection.corners).astype(int)
        cv2.polylines(overlay, [corners], True, (0, 255, 0), 2)

        center = tuple(numpy.asarray(detection.center).astype(int))
        ...
        label = '{}:{}'.format(family, detection.tag_id)
        cv2.putText(overlay, label, center,
                    cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 255), 1, cv2.LINE_AA)


def _print_pupil_detection(detection, with_pose=False):
    ...
    print('  ID: {}'.format(detection.tag_id))
    print('  Hamming: {}'.format(detection.hamming))
    print('  DecisionMargin: {}'.format(detection.decision_margin))
    print('  Center: {}'.format(detection.center))
    print('  Corners: {}'.format(detection.corners))

    if with_pose and hasattr(detection, 'pose_R') and hasattr(detection, 'pose_t'):
        print('  Pose_R:')
...
def main():
    ...
    try:
        ...
        try:
            ...
            raise RuntimeError(
                'apriltag shared library not found and pupil_apriltags is not installed. '
        ...
        det = PupilDetector(
            ...
            quad_decimate=float(options.quad_decimate),
            quad_sigma=float(options.quad_sigma),
            refine_edges=bool(options.refine_edges),
            decode_sharpening=0.25,
        )
        print('Native apriltag shared library not found, using pupil_apriltags fallback backend.')
    ...
    for filename in options.filenames:
        ...
        if backend == 'ctypes':
            ...
            _draw_pupil_detections(overlay, detections)
        ...
        for i, detection in enumerate(detections):
            ...
            if backend == 'ctypes':
                ...
                _print_pupil_detection(detection,
                                       with_pose=(options.camera_params is not None))

            if options.camera_params is not None and backend == 'ctypes':
            ...
            if options.camera_params is not None and backend != 'ctypes':
                if hasattr(detection, 'pose_R') and hasattr(detection, 'pose_t'):
                    pose = numpy.eye(4)
                    pose[:3, :3] = detection.pose_R
                    pose[:3, 3] = numpy.asarray(detection.pose_t).reshape(3)
                    if _HAVE_CV2:
                        _draw_pose(overlay,
                                   options.camera_params,
                                   options.tag_size,
